...
class packet_struct:
    def init_from_binary(self, bin_trace):
        ...
        self.GMTtime = hex_trace[0]
        self.MicroTime = hex_trace[1]
        self.caplen = hex_trace[2]
        self.len = hex_trace[3]

    # TCP是6
    # UDP是17
    def init_from_pacp(self, src_ip, src_port, dst_ip, dst_port, proto, second, microSecond ,time):
    ...
    def __str__(self):
        return "%s %s %s %s %s %s %s %s" % (self.src_ip, self.src_port, self.dst_ip, self.dst_port, self.proto,self.second,self.microSecond ,self.time)
